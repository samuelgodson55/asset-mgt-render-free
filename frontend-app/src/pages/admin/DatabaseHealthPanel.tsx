...
const DEMO_SNAPSHOT: DbPoolDiagnostics = {
  ...
  pgbouncer_pool: { cl_active: 3, cl_waiting: 0, sv_active: 3, sv_idle: 7, sv_used: 3, maxwait_seconds: 0, avg_query_time_us: 12000, avg_wait_time_us: 0 },
...
function healthFor(snapshot: DbPoolDiagnostics | null): { level: HealthLevel; label: string; detail: string } {
  ...
  const attention = postgresPct >= 75 || (b?.cl_waiting ?? 0) > 0 || (pool.overflow ?? 0) > 0 || (pg.idle_in_transaction ?? 0) > 0 || (b?.avg_wait_time_us ?? 0) > 0;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <section className="rounded-[5px] border border-border-soft bg-surface p-5">
          <div className="flex items-center gap-2 mb-4"><Waves size={16} className="text-moss-soft" /><h3 className="font-display text-sm font-semibold text-text">PgBouncer</h3></div>
          <div className="grid grid-cols-2 gap-x-5 gap-y-4 text-[11px]">
