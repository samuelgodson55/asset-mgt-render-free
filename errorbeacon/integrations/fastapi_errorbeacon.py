...
ERRORBEACON_URL = os.getenv("ERRORBEACON_URL", "http://errorbeacon:8000")
ERRORBEACON_API_KEY = os.getenv("ERRORBEACON_INGEST_API_KEY", "") or os.getenv("ERRORBEACON_API_KEY", "")
ERRORBEACON_APP = os.getenv("ERRORBEACON_APP", "asset-inventory-quotes")
ERRORBEACON_ENVIRONMENT = os.getenv("ENVIRONMENT", "production")
ERRORBEACON_RELEASE = os.getenv("APP_RELEASE", "")
ERRORBEACON_TIMEOUT = float(os.getenv("ERRORBEACON_TIMEOUT", "0.75"))
...
def send(payload: dict[str, Any]) -> None:
    ...
    try:
        headers = {"X-API-Key": ERRORBEACON_API_KEY}
        request_id = payload.get("request_id")
        if request_id:
            headers["X-Request-ID"] = str(request_id)[:200]

        requests.post(
            f"{ERRORBEACON_URL.rstrip('/')}/v1/events",
            json=payload,
            headers=headers,
            timeout=(0.1, ERRORBEACON_TIMEOUT),
        )
...
def report_exception(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "app": ERRORBEACON_APP,
            "environment": ERRORBEACON_ENVIRONMENT,
            "severity": severity,
            ...
            "release": ERRORBEACON_RELEASE or None,
