...
class UserConvertToOutsiderRequest(BaseModel):
    ...
    convert_user_to_outsider() sensibly defaults `contact_details` from
    the account being converted (its existing email) when omitted.
    `company` has no automatic default (a user's `department` is an
    internal team, not an external company, so it would be misleading to
    silently reuse it) -- leave it blank if the person isn't affiliated
    with an outside company. `name` is deliberately NOT a field here,
    same reasoning as OutsiderConvertToUserRequest.email being tied to
    the source profile -- this is that same person losing their login,
    not a chance to rename them mid-conversion.
    """
    contact_details: Optional[str] = None
    company: Optional[str] = None

    # Same "a present-but-blank value would silently violate NOT NULL"
    # guard as OutsiderUpdateRequest.name/contact_details -- contact_details
    # is required on models.Outsider, so an explicit empty string here
    # would just produce a confusing failure deeper in the service layer.
    @field_validator("contact_details")
    @classmethod
    def _reject_blank_contact(cls, value: Optional[str]) -> Optional[str]:
        if value is not None and not value.strip():
            raise ValueError("This field cannot be blank.")
        return value.strip() if value is not None else value
