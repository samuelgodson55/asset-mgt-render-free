        ...
        rows.append(
            {
                ...
                "department": asset_type.department,
...
# ---------------------------------------------------------------------------
# 3. SPEND BY CATEGORY / USER DEPARTMENT (LEGACY COMPATIBILITY)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 4. RENTAL REVENUE BY ASSET DEPARTMENT
# ---------------------------------------------------------------------------
def get_revenue_by_asset_department(
    db: Session,
    start_date: "Optional[datetime.date]" = None,
    end_date: "Optional[datetime.date]" = None,
) -> dict:
    """
    Rental revenue generated by fulfilled or paid quotations, grouped by the
    department assigned to each catalog asset pool (e.g. Camera, Lighting,
    Grip). This intentionally does NOT use AssetCheckout.quantity *
    AssetType.price: that older "spend" metric measures replacement/unit
    value and can be unrelated to rental revenue.

    Revenue is the rental charge before VAT, after the quote-level discount.
    VAT is a tax collected on behalf of government, not asset revenue. A
    quotation only contributes after it reaches ``fulfilled`` (and remains
    included when it later moves to terminal ``paid``) and its
    ``fulfilled_at`` falls inside the requested window. Outsourced quote
    lines are excluded from asset-department revenue because they have no
    AssetType department. Assets without a department are grouped under
    ``Unassigned`` so missing classification is visible rather than silently
    disappearing from the report.
    """
    start_dt, end_dt = _date_range_bounds(start_date, end_date)
    query = db.query(models.Quotation).filter(models.Quotation.status.in_(["fulfilled", "paid"]))
    if start_dt is not None:
        query = query.filter(models.Quotation.fulfilled_at >= start_dt)
    if end_dt is not None:
        query = query.filter(models.Quotation.fulfilled_at < end_dt)
    ...
    for quotation in quotations:
        # Quote pricing follows quotation_service._serialize_quotation():
        # live AssetType.price * quantity * inclusive rental days.
        item_rows = []
        gross_subtotal = 0.0
        for item in quotation.items:
            ...
            if asset is None:
                continue
            unit_price = float(asset.price) if asset.price is not None else 0.0
            days = max(1, (item.due_date - item.start_date).days + 1)
            gross = unit_price * item.quantity * days
            item_rows.append((asset, gross, item.quantity))
        ...
        for asset, gross, quantity in item_rows:
            if asset.price is None:
                continue
            revenue = gross * discount_factor
            department = (asset.department or "Unassigned").strip() or "Unassigned"
            bucket = buckets.setdefault(department, {"total_revenue": 0.0, "item_count": 0, "quotation_count": set()})
            bucket["total_revenue"] += revenue
            bucket["item_count"] += quantity
            bucket["quotation_count"].add(quotation.id)
            total_revenue += revenue
            priced_line_count += 1
            if department == "Unassigned":
                unassigned_line_count += 1

    rows = []
    for department, bucket in buckets.items():
        rows.append({
            "department": department,
            "total_revenue": round(bucket["total_revenue"], 2),
            "item_count": bucket["item_count"],
            "quotation_count": len(bucket["quotation_count"]),
        })
    ...
    return {
        ...
        "fulfilled_quotation_count": len(quotations),
        "priced_line_count": priced_line_count,
        "unassigned_line_count": unassigned_line_count,
    }


# ---------------------------------------------------------------------------
# 5. QUOTATION APPROVAL TURNAROUND TIME
# ---------------------------------------------------------------------------
...
def get_dashboard(
    ...
    """
    Single-call bundle of all reporting sections for the Reporting dashboard's
    ...
    return {
        ...
        "revenue": get_revenue_by_asset_department(db, start_date, end_date),
