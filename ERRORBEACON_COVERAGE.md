...
## v6 hardening implemented

The final ErrorBeacon pass adds the following protections across the integrated application:

- Bounded client-side reporting queue instead of one thread per exception.
- Bounded ErrorBeacon alert queue with fixed worker threads.
- Recursive secret redaction before persistence, Telegram delivery or Gemini analysis.
- Redaction of JWTs, Bearer tokens, database URLs, credentials and sensitive context keys.
- Browser URL sanitization for reset/access/refresh tokens, codes, passwords, API keys and session identifiers.
- Browser request-ID propagation from `X-Request-ID`, including raw `fetch()` calls.
- Persistent error-rate events for restart-safe spike detection.
- Telegram inline View, Resolve and Silence-for-one-hour controls.
- Telegram callback authorization limited to the configured monitoring chat.
- ACA Azure Files persistence for `/data/errorbeacon.db`.
- SQLite schema initialization/migrations moved to application startup.
- A root-level-safe ErrorBeacon test setup.

The controlled `/v1/test` response returns the request ID that is displayed in Telegram, making correlation easy to verify.
